...
func (r *leaveRepository) CreateLeaveRequest(request model.LeaveRequest) error {
	query := `INSERT INTO leave_requests (org_id, employee_id, leave_category, start_date, end_date, reason, status, approved_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
...
func (r *leaveRepository) UpdateLeaveRequestStatus(orgID, leaveRequestID int, status, updatedBy string) error {
	updateStatusQuery := `UPDATE leave_requests SET status = ?, approved_by = ?, updated_at = ? WHERE leaveRequestID = ? and org_id = ?`
...
func (r *leaveRepository) GetLeavesInRange(orgID int, startDate, endDate time.Time) ([]model.LeaveRequest, error) {
	query := `SELECT id, org_id, employee_id, leave_category, start_date, end_date, reason, status, approved_by, created_at, updated_at FROM leave_requests WHERE org_id = ? AND start_date >= ? AND end_date <= ?`
